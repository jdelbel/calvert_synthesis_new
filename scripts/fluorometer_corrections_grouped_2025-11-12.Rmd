...
ggsave(here("figures", "ctd_fluorescence_calibration_by_sensor_all.png"),
       width = 10,
       height = 6.5,
       dpi = 300,
       bg = "white")
...
fit_stats <- f_fit_av %>% 
  ...
  summarise(
    ...
    slope_low = if(first(ctdNum) == "80217") {
      coef(lm(chl_comb ~ f_dm, data = pick(everything()) %>% filter(chl_comb < 1)))[2]
    } else {
      NA_real_
    },
    intercept_low = if(first(ctdNum) == "80217") {
      coef(lm(chl_comb ~ f_dm, data = pick(everything()) %>% filter(chl_comb < 1)))[1]
    } else {
      NA_real_
    },
    r2_low = if(first(ctdNum) == "80217") {
      summary(lm(chl_comb ~ f_dm, data = pick(everything()) %>% filter(chl_comb < 1)))$r.squared
    } else {
      NA_real_
    },
    adj_r2_low = if(first(ctdNum) == "80217") {
      summary(lm(chl_comb ~ f_dm, data = pick(everything()) %>% filter(chl_comb < 1)))$adj.r.squared
    } else {
      NA_real_
    },
    p_value_low = if(first(ctdNum) == "80217") {
      summary(lm(chl_comb ~ f_dm, data = pick(everything()) %>% filter(chl_comb < 1)))$coefficients[2, 4]
    } else {
      NA_real_
    },
...
plot_cast_detailed <- function(cast_id) {
  ...
  profile_plot <- profile_data %>%
    filter(pres < 50) %>%
    ggplot(aes(y = pres)) +
    geom_ribbon(aes(xmin = f_lower, xmax = f_upper), 
...
```{r}
# Usage
plot_cast_detailed("2021-04-10-KC10")
...
```{r}
f_fit %>% 
  filter(id == "2016-08-12-FZH01-18066") %>% 
  # filter(!pres == 1) %>% 
  ggplot(aes(x = f_dm, y = chl_comb, color = as.factor(pres))) +
  geom_point()
```
...
```{r}
f %>% 
  filter(date == "2016-08-12-FZH01" & station == "FZH01") %>% 
...
```{r}
f_dm %>% 
  filter(id == "2014-09-11-FZH01-80217") %>% 
  ggplot(aes(y = pres)) +
...
test <- f_fit_few %>% 
  filter(ctdNum == "18066") %>% 
  filter(f_dm > 10 & chl_comb < 7)

test2 <- f_fit_few %>% 
  filter(ctdNum == "18066") %>% 
  filter(f_dm > 5 & chl_comb < 3)
...
ggsave(here("figures", "fluorescence_calibration_by_sensor_with_single.png"),
       width = 12, height = 10, dpi = 300)
```
