...
f_fit_multi <- f_fit %>%
  ...
  ungroup() %>%
  filter(!(id == "2023-03-18-KC10" & pres == 30)) %>% 
  filter(!(id == "2014-06-19-KC10" & pres == 5)) %>% 
  filter(!(id == "2017-08-31-KC10" & pres == 1)) %>% 
  filter(!(id == "2018-07-15-FZH01" & pres == 10)) %>% 
  filter(!(id == "2017-04-28-FZH01" & pres == 10)) %>% 
  filter(!(id == "2017-07-19-FZH01" & pres == 20)) %>% 
  filter(!(id == "2017-09-21-FZH01" & pres == 10))
...
ggsave(here("figures", "fluorescence_calibration_by_sensor_comp2.png"),
...
```{r}
# Usage
# plot_cast_detailed("2016-08-12-FZH01")
...
plot_cast_detailed <- function(cast_id) {
  ...
  profile_plot <- profile_data %>%
    ...
    ggplot(aes(y = pres)) +
    geom_ribbon(aes(xmin = f_lower, xmax = f_upper), 
                fill = "#2E86AB", alpha = 0.3) +  # shaded band
    geom_line(aes(x = f_dm, color = "CTD Fluorescence"), 
              linewidth = 1, orientation = "y") +
    geom_point(data = cast_data %>% filter(pres < 50),
               aes(x = chl_comb, color = "Discrete Samples"), 
