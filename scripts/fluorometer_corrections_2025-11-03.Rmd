...
```
```{r}
#Writing basic QC output

write.csv(f, here("outputs", "flu_kc10_fzh1_qc1_2025-11-13.csv"))
...
f_fit %>% 
  filter(pres > 1) %>% 
  filter(date == "2023-08-16") %>% 
  ggplot(aes(y = chl_comb, x = f_dm)) +
  geom_point()

```

```{r}
# PROFILE PLOT - Nice styling with legend
profile_plot <- f_dm %>% 
  filter(pres < 50) %>% 
  filter(date == "2023-05-14") %>% 
  ggplot(aes(y = pres)) +
...
# Filter data (excluding pres == 1 for regression)
f_fit_filtered <- f_fit %>% 
  filter(date == "2023-05-14") %>% 
  filter(pres != 1)

# Calculate regression statistics
model <- lm(chl_comb ~ f_dm, data = f_fit_filtered)
...
# Create equation and stats text
eq_label <- sprintf("y = %.3fx + %.3f\nR² = %.3f\np < %.4f", 
                    slope, intercept, r_squared, p_value)

# Get data for this date to determine axis limits
plot_data <- f_fit %>% 
  filter(date == "2023-05-14")

# Calculate common axis limits
axis_min <- min(c(plot_data$f_dm, plot_data$chl_comb), na.rm = TRUE)
axis_max <- max(c(plot_data$f_dm, plot_data$chl_comb), na.rm = TRUE)
# Add a small buffer (5%)
buffer <- (axis_max - axis_min) * 0.05
axis_limits <- c(axis_min - buffer, axis_max + buffer)

# SCATTER PLOT with discrete pressure colors
scatter_plot <- plot_data %>% 
  ...
  geom_abline(intercept = 0, slope = 1, 
              ...
              linewidth = 0.8) +
  geom_point(size = 3) +
  geom_smooth(data = f_fit_filtered, 
  ...
  labs(
    ...
    subtitle = "Regression excludes pres = 1; dashed line = 1:1"
  ) +
  annotate("text", 
           x = axis_limits[1], 
           y = axis_limits[2], 
...
combined_plot <- profile_plot + scatter_plot +
  plot_annotation(
    title = "Fluorescence Comparison - 2023-05-14",
...
# Save to figures directory using here package
ggsave(
  filename = here("figures", "fluorescence_comparison_2023-05-14.png"),
  plot = combined_plot,
  width = 12,
  height = 6,
  dpi = 300,
...
# Filter data (excluding pres == 1)
f_fit_filtered <- f_fit %>% 
  filter(date == "2023-05-14") %>% 
  filter(pres != 1)

# Replace the 20m bulk value with the fractionated value
f_fit_modified <- f_fit_filtered %>%
  mutate(chl_comb = if_else(pres == 20, 17.09, chl_comb))

# Create regression dataset excluding 10m outlier
f_fit_regression <- f_fit_modified %>%
  filter(pres != 10)

# PROFILE PLOT - with modified 20m value
profile_plot <- f_dm %>% 
  filter(pres < 50) %>% 
  filter(date == "2023-05-14") %>% 
  left_join(
    f_fit_modified %>% select(pres, chl_comb_modified = chl_comb),
    by = "pres"
  ) %>%
  ggplot(aes(y = pres)) +
  geom_line(aes(x = f_dm, color = "CTD Fluorescence"), 
            linewidth = 1, orientation = "y") +
  geom_point(aes(x = chl_comb_modified, color = "Discrete Samples"), 
             size = 3, shape = 16) +
  scale_y_reverse(name = "Pressure (dbar)") +
  scale_x_continuous(name = "Chlorophyll (µg/L)") +
  scale_color_manual(
...
# Calculate regression statistics excluding 10m
model <- lm(chl_comb ~ f_dm, data = f_fit_regression)
model_summary <- summary(model)
r_squared <- model_summary$r.squared
slope <- coef(model)[2]
intercept <- coef(model)[1]
...
# Calculate 5m slope factor (chl_comb/f_dm ratio)
slope_5m <- f_fit_modified %>% 
  filter(pres == 5) %>% 
  mutate(ratio = chl_comb / f_dm) %>% 
  pull(ratio)
...
eq_label_5m <- sprintf("5m slope factor: %.3f", slope_5m)

# Calculate common axis limits
axis_min <- min(c(f_fit_modified$f_dm, f_fit_modified$chl_comb), na.rm = TRUE)
axis_max <- max(c(f_fit_modified$f_dm, f_fit_modified$chl_comb), na.rm = TRUE)
# Add a small buffer (5%)
buffer <- (axis_max - axis_min) * 0.05
axis_limits <- c(axis_min - buffer, axis_max + buffer)

# SCATTER PLOT with modified 20m value and 10m as outlier
scatter_plot <- f_fit_modified %>% 
  ggplot(aes(x = f_dm, y = chl_comb)) +
  geom_abline(intercept = 0, slope = 1, 
              linetype = "dashed", 
              color = "gray50", 
              linewidth = 0.8) +
  # Regression line excluding 10m
  geom_smooth(data = f_fit_regression,
              aes(x = f_dm, y = chl_comb),
              method = "lm", 
              se = TRUE, 
              color = "black", 
  ...
  geom_point(data = f_fit_modified %>% filter(pres != 10),
             aes(color = as.factor(pres)),
             size = 3) +
  # 10m outlier point with different shape
  geom_point(data = f_fit_modified %>% filter(pres == 10),
             aes(x = f_dm, y = chl_comb),
             shape = 1,  # Open circle
             size = 4,
             stroke = 1.5,
             color = "#8DA0CB") +  # 10m color from Set2 palette
  scale_color_brewer(
    name = "Pressure\n(dbar)",
    palette = "Set2"
  ) +
  ...
  labs(
    title = "B) CTD vs Discrete Chlorophyll",
    subtitle = "20m = fractionated; 10m (open circle) = outlier excluded; dashed line = 1:1"
  ) +
  annotate("text", 
           x = axis_limits[1], 
           y = axis_limits[2], 
...
# Save to figures directory using here package
ggsave(
  filename = here("figures", "fluorescence_comparison_2023-05-14_20m_sf_10_out.png"),
  plot = combined_plot,
  width = 12,
  height = 6,
  dpi = 300,
...
# Filter data (excluding pres == 1) and replace 20m value on 2023-05-14
f_fit_filtered <- f_fit %>% 
  filter(pres > 1) %>% 
  filter(date >= "2023-05-14" & date <= "2023-11-25") %>%
  mutate(chl_comb = if_else(date == as.Date("2023-05-14") & pres == 20, 17.09, chl_comb))

# Create regression dataset excluding 2023-05-14 10m outlier
f_fit_regression <- f_fit_filtered %>%
  filter(!(date == as.Date("2023-05-14") & pres == 10))

# Identify the outlier point for separate plotting
f_fit_outlier <- f_fit_filtered %>%
  filter(date == as.Date("2023-05-14") & pres == 10)

# Calculate regression statistics excluding the outlier
model <- lm(chl_comb ~ f_dm, data = f_fit_regression)
model_summary <- summary(model)
r_squared <- model_summary$r.squared
slope <- coef(model)[2]
...
# Create equation and stats text
eq_label <- sprintf("y = %.3fx + %.3f\nR² = %.3f\np < %.4f\nn = %d", 
                    slope, intercept, r_squared, p_value, nrow(f_fit_regression))

# Calculate common axis limits
axis_min <- min(c(f_fit_filtered$f_dm, f_fit_filtered$chl_comb), na.rm = TRUE)
axis_max <- max(c(f_fit_filtered$f_dm, f_fit_filtered$chl_comb), na.rm = TRUE)
# Add a small buffer (5%)
buffer <- (axis_max - axis_min) * 0.05
axis_limits <- c(axis_min - buffer, axis_max + buffer)

# Create scatter plot with date as factor for discrete colors
scatter_plot <- f_fit_regression %>% 
  ggplot(aes(x = f_dm, y = chl_comb)) +
  geom_abline(intercept = 0, slope = 1, 
              linetype = "dashed", 
              color = "gray50", 
              linewidth = 0.8) +
  # Points included in regression
  geom_point(aes(color = as.factor(date)), size = 3, alpha = 0.8) +
  # Outlier point as open circle
  geom_point(data = f_fit_outlier,
             aes(x = f_dm, y = chl_comb),
             shape = 1,
             size = 4,
             stroke = 1.5,
  ...
  labs(
    title = "Discrete Chlorophyll vs CTD Fluorescence",
    subtitle = "May 14 - Nov 25, 2023 (pres > 1); open circle = outlier (2023-05-14 10m); dashed = 1:1"
  ) +
  annotate("text", 
           x = axis_limits[1], 
           y = axis_limits[2], 
...
# Save to figures directory using here package
ggsave(
  filename = here("figures", "fluorescence_comparison_multi-date_2023.png"),
  plot = scatter_plot,
  width = 8,
  height = 7,
  dpi = 300,
...
combined_plot <- profile_plot + scatter_plot +
  plot_annotation(
    title = "Fluorescence Comparison - 2023-07-18",
    theme = theme(plot.title = element_text(size = 16, face = "bold"))
  )
...
# Save to figures directory using here package
ggsave(
  filename = here("figures", "fluorescence_comparison_2023-07-18.png"),
  plot = combined_plot,
  width = 12,
  height = 6,
  dpi = 300,
