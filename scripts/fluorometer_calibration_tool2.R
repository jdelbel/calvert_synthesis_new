...
plot_cast_detailed <- function(cast_id) {
  ...
  # Get full CTD profile data from f (not f_dm)
  profile_data <- f %>%
  ...
  # Flag status text
  flag_text <- paste(
    ifelse(qc_status$auto_flag, "AUTO-FLAGGED", ""),
    ifelse(qc_status$user_flag, "USER-FLAGGED", ""),
    sep = " "
  )
  if (flag_text == " ") flag_text <- "NOT FLAGGED"
  
  # PROFILE PLOT - using full CTD profile from f
  profile_plot <- profile_data %>%
    filter(pres < 50) %>%
    ggplot(aes(y = pres)) +
    geom_line(aes(x = flu, color = "CTD Fluorescence"), 
  ...
  combined_plot <- profile_plot + scatter_plot +
    plot_annotation(
      ...
      subtitle = if(nchar(qc_status$qc_comment) > 0) {
        paste0("Comment: ", qc_status$qc_comment)
      } else {
        NULL
      },
      theme = theme(
...
# ==============================================================================
# STEP 9: Interactive browsing function
# ==============================================================================

browse_casts <- function(flagged_only = FALSE) {
  # Get cast list
  if (flagged_only) {
    cast_list <- model_flagged %>% 
      filter(auto_flag) %>% 
      pull(id)
    cat(paste0("\n=== Browsing ", length(cast_list), " auto-flagged casts ===\n"))
  } else {
    cast_list <- model_flagged %>% 
      filter(!is.na(slope)) %>%
      pull(id)
    cat(paste0("\n=== Browsing all ", length(cast_list), " casts ===\n"))
  }
  ...
  repeat {
    ...
    # Get user input
    cat("\nCommands:\n")
    cat("  [n]ext | [p]revious | [s]ave plot | [f]lag/unflag | [c]omment | [q]uit\n")
    ...
    if (input == "n" || input == "") {
      ...
      ggsave(here("figures", filename),
             plot = p, width = 12, height = 6, dpi = 300, bg = "white")
      cat(paste0("Saved to: ", here("figures", filename), "\n"))
    } else if (input == "f") {
      # Toggle flag
