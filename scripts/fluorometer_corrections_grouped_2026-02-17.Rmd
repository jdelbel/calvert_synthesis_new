...
#I use this to evaluate why a profile/discrete sample matchup may be poor or show a divergent offset or slope. Sometimes this appears to happen with depth mismatches between fluorescence and discrete concentrations - particularly where there are sharp peaks or SCM
...
#Portal file of all KC10 and FZH01 profiles - raw data
f <- read_csv(here("files", "8_binAvg-1762199720021.csv"))

#CTD fluorescence profiles flags, determined using my visual profile QC tool
flags <- read_csv(here("outputs","fluorescence_QC_flags_20251103.csv"))

#Chlorophyll data downloaded from the Portal for KC10/FZH01
c <- read_csv(here("files", "2025-11-03_HakaiData_chlorophyll.csv"))

#HPLC data downloaded from the portal from KC10/FZH01 for comparison with chlorophyll
hplc <- read.csv(here("files", "2025-11-18_HakaiData_hplc.csv"))

# Downloading my fit flagging sheet - this was a manual sheet created by looking at profile by profile discrete chlorophyll to fluorescence scatters to identify poor relationships. I want to try to create thresholds for slope and intercept.
fit_flag <- read_csv(here("outputs", "ctd_discrete_flags5.csv"))

#QC nightly median fluorescence data - not to date.
...
```{r}
#Two high biomass profiles showed CTD instrument saturation at around 5 mg m-3. These are the two dates. Pulling these out as I correct them later.
f_saturate <- f %>% 
  filter(date %in% c("2022-03-09", "2022-09-09"))

#Pulling out the removed profiles
f_issue <- f %>% 
  left_join(flags) %>% 
  filter(!flag == "AV") 

#Removing bad or suspect profiles, but leaving shallow casts. I think I define shallow casts as those that do not exceed 100m.
f <- f %>% 
  left_join(flags) %>% 
  filter(flag == "AV") %>% 
...
#Plotting the removed profiles.
ggplot(f_issue %>% filter(pres < 100), aes(x = flu, y = pres)) +
  geom_line(orientation = "y") +
  geom_point() +
  geom_text(
    data = f_issue %>% filter(pres < 100) %>% group_by(castpk) %>% slice(1),
    aes(label = comment),
    x = Inf, y = Inf,
    hjust = 1.1, vjust = 1.5,
    size = 3
  ...
  labs(x = "Fluorescence", y = "Pressure (dbar)")

ggsave(
  here("figures", "fluorescence_issue_profiles.png"),
  width = 10, height = 8, dpi = 300
)
```

```{r}
#Looking at days with replicate casts and looking at the time difference between them. Currently (below), I simply do a daily average for replicate casts and samples and that could certainly add variability or degrade relationships.
num_dup <- f %>% 
  filter(pres == 5) %>%
...
#Later the relationship from this cast is made SVD because of apparent depth mismatches - I wonder if picking by closest time would make better?

#2017-08-31-FZH01 - Some differences here for-sure.
#variability is largely in the surface waters and I did have to cut these out for the final ctd wide relationship.
...
dark_offsets <- f %>%
  filter(pres > 100) %>% #deeper than 100m
  group_by(castpk, date, station, ctdNum) %>% #Why am i not also using castpk?
  mutate(n = n()) %>% 
  ungroup() %>% 
  filter(n >= 10) %>% 
...
# Looking at early time-series casts that did not go deep enough to derive a deep dark offset - can shallower data be used? The answer is no -  really need deep data.
shallow_dark <- f %>% 
...
```{r}
#Plotting the dark offsets with the shallow casts -  this highlights that shallow derived dark values are often too high.
dark_offsets %>% 
  ggplot(aes(x = date, y = dark_offset, color = as.factor(ctdNum))) +
  # Deep offsets
  geom_errorbar(aes(ymin = dark_offset - dark_offset_sd, 
...
```{r}
#Investigate the few profiles where there are high offsets in the deep >100m derived data.

#Here, looking at 80217 > 1
...
```{r}
#Fill early time-series shallow casts with closes deep dark offset - should be ok as the sensors this is being done too are relatively stable and show low variability. Could introduce error, but these corrections are marginal especially during the high biomass events we are interested in.
f <- f %>% 
...
#This subtracts the dark offset from the fluorescence value at each depth
f_offset <- f %>% 
...
#For both Bulk and size-fractionated, I am allowing SVC and ADL and not flagged. Usually with SVC the data look OK, but had a about something that happened with the sample. ADL is trickier - this is for samples from before we started diluting when the raw value exceeded the max value from the calibration, which were done spanning the instrument range. In theory, these should skew low, but it's not apparent. 

#Filtering out the bulk samples meeting the thresholds
c_qc <- c %>% 
...
#Calculating a daily mean value in case of duplicates, rounding out any weird depths likely created by data entry error and setting the 0m sampling depth to 1m to match with the closest CTD fluorometer record
c_dm <- c_qc %>% 
...
#Pulling out size-fractionated data that meet the thresholds.
c_sf <- c %>% 
  select(date, station = site_id, line_out_depth, filter_type, chla, chla_flag) %>% 
  filter(!filter_type == "Bulk GF/F") %>% 
  filter(chla_flag == "AV" | chla_flag == "SVC" | chla_flag == "ADL" | is.na(chla_flag))

#Calculating a daily mean value, making sure all three filters types are available to complete the set and then calculating the sum. We switched the middle filter size from 2-3um, but shouldn't matter here. 
c_sf_dm <- c_sf %>% 
  filter(!is.na(chla)) %>%
  filter(chla > 0) %>% 
  group_by(date, station, line_out_depth, filter_type) %>% 
  summarise(avg_chla = mean(chla)) %>% #daily mean by filter type
  ungroup() %>% 
  group_by(date, station, line_out_depth) %>% 
  mutate(n_filt = n()) %>% #count of filters in each set
  ungroup() %>% 
  group_by(date, station, line_out_depth, filter_type) %>% 
  mutate(n_type = n()) %>% #identify duplicate filter types
  ungroup() %>% 
  filter(n_filt == 3 & n_type == 1) %>% #only use data with 3 different types
  group_by(date, station, line_out_depth) %>% 
  mutate(sum = sum(avg_chla)) %>% #size-fractionated sum
  ungroup() %>% 
...
#Setting the 0m sampling depth to 1m for merging with the CTD.
c_sum <- c_sf_dm %>% 
...
```{r}
#Joining the bulk and size-fractionated sum and then creating combined column where the size-fractionated value is used where there are NA's for the bulk. This fills out the dataset for the most possible comparisons.
c_join <- c_dm %>%
...
```{r}
# Doing a comparison of the size-fractionated sum and bulk to show that they are mostly inter-comparable.

# Remove NAs and classify outliers based on large differences. Probably a better approach, but I think fine for here. Not uncommon to have difference in replicates for chlorophyll data, even without sample issues, do to aggregates/large cells.
c_join_comp <- c_join %>%
...
# Create scatter plot showing the size-fractionated - bulk comparison.
#The slope is 0.89 showing slightly higher concentrations for the bulk - this could be due to pigment loss going through the stacked filters.
c_join_comp %>% 
...
```{r}
#Now doing a comparison between the combined chl product (SF sum where no Bulk available) with HPLC tchla

#Pulling out the relevent columns, doing a daily mean for replicates and changing the 0m depth to 1m for merging with the CTD data.
hplc <- hplc %>%  
...
# Create scatterplot between HPLC tchla and combined chlorophyll
#Quite scattered with chlorophyll much higher than HPLC (slope = 1.55). Not uncommon.
p_hplc <- c_join_hplc %>% 
...
```{r}
#Joining the combined chlorophyll dataset with the CTD fluorometer data

#Is this needed?
f_dm <- f_dm %>% 
  left_join(c_join)

#when used, the SF sum data could have the offset applied.
#Using HPLC would be better, but not nearly as many samples and only goes back to 2017ish.
```



```{r}
#Creating a dataset to apply linear fits between the chlorophyll and fluorometry data to derive a slope correction.

#I am manually removing large outliers determined from visual inspection. They are all typically from the discrete samples likely missing a narrow SCM layer below.

#Joining the CTD and discrete data, removing any NAs (only where data matches exist) and creating a unique ID.
f_fit <- f_dm %>%
  left_join(c_join) %>% 
...
# Separate into multi-sample (>=3) and few-sample (1-2) casts (generally earlier)
# The filtered out data are large outliers that were identified via visual inspection and judgement calls.
# Notes on these outlier removals are in excel sheet and f_flag dataframe under comments
# Not sure if they should be flagged as bad data as they aren't necessarily bad 
f_fit_multi <- f_fit %>%
  filter(!is.na(chl_comb) & !is.na(f_dm)) %>%
...
```{r}
#Function to perfoming fitting on casts by cast basis both including and excluding the surface value based on p.value and r2 - this is done because the CTD fluorescence surface value can be quenched - artificially low - and this allows the other data to be used to derive a fit. 
...
```{r}
# Joining the model results back in with the CTD data to get CTDNum back - need this for looking at sensor wide comparisons
...
# Get median slope and intercept by sensor - use this as a baseline for looking at variability.
...
```{r}
# Creating scatter plots of fluorescence versus discrete concentrations by CTD/sensor using all available data
...
```{r}
#Creating scatterplots using only the best matches based on manual inspection/QC

#fit flag has all of my manual flags based on my cast by cast inspections - this was really time consuming and usually issues were apparent, but not always, causing judgement calls.

#Setting up the QC flags for merging.
fit_flag_clean <- fit_flag %>% 
  unite(id, c(id, ctdNum), sep = "-", remove = F) %>% 
  select(id,
         slope_flag = flag)

#Pulling out profiles where quenching appeared to be an issue.
fit_flag_quench <- fit_flag %>% 
  unite(id, c(id, ctdNum), sep = "-", remove = F) %>%
  select(id, quenched) %>% 
  filter(quenched == "y")

#Joining the flags with the data and only selecting the data I flagged as AV.
f_fit_av <- f_fit_with_model %>% 
  left_join(fit_flag_clean) %>% 
  filter(slope_flag == "AV")

# Calculate overall fit statistics for each sensor using only the best data.
fit_stats <- f_fit_av %>% 
...
#Plotting the scatterplot and regressions
f_fit_av %>% 
  filter(use_surface | pres != 1) %>%
