...
```{r}
# vertical_metrics <- ctd_fl %>%
#   filter(pres >= 0 & pres <= 30) %>%  # or your euphotic zone depth
#   group_by(id, date, station) %>%
#   arrange(id, pres) %>%
#   summarise(
#     # Integrated chlorophyll (if you have fluor data)
#     int_chla = pracma::trapz(pres, f_final),
#     
#     # SCM metrics
#     scm_depth = pres[which.max(f_final)],
#     scm_max = max(f_final, na.rm = TRUE),
#     surface_chla = mean(f_final[pres >= 1 & pres <= 5], na.rm = TRUE),
#     scm_enhancement = scm_max / surface_chla,
#     scm_perc = scm_max/int_chla,
#     
#     # Distribution metrics
#     centroid_depth = sum(pres * f_final) / sum(f_final),
#     biomass_top10m = sum(f_final[pres <= 10]) / sum(f_final),
#     .groups = 'drop'
#   )
...
vertical_metrics <- ctd_fl %>%
  ...
  summarise(
    ...
    # 1. Surface depletion (0 = no SCM, 1 = extreme SCM)
    surface_chla = mean(f_final[pres >= 1 & pres <= 5], na.rm = TRUE),
    scm_max = max(f_final, na.rm = TRUE),
    surface_depletion = (scm_max - surface_chla) / scm_max,
    
    # 2. Profile shape (CV = coefficient of variation)
    # High = peaked/concentrated, Low = uniform
    profile_shape = sd(f_final) / mean(f_final),
    
    # 3. Vertical gradient strength (average rate of change)
    # High = sharp transitions, Low = gradual/mixed
    vertical_gradient = mean(abs(diff(f_final)) / diff(pres), na.rm = TRUE),
...
```{r}
vertical_export <- vertical_metrics %>% 
  select(date,
         station,
         int_chla,
         scm_depth,
  ...
  distinct() %>% 
  group_by(date) %>% 
  summarise(int_chla = mean(int_chla),
            scm_depth = mean(scm_depth),
            centroid_depth = mean(centroid_depth),
            surface_depletion = mean(centroid_depth),
            profile_shape = mean(profile_shape),
            vertical_gradient = mean(vertical_gradient),
            .groups = "drop") 

write_csv(vertical_export, here("outputs", "vertical_metrics.csv"))
```
