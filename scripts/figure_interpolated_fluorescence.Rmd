...
ctd_fl <- read_csv(here("outputs",
                        "ctd_offset_slope_corrected_fluorescence_2025-11-19.csv"))
...
interp <- as.data.frame(sp.interp) %>% 
  ...
  mutate(date = as.Date(date),
         station = "KC10",
         year = year(date),
         f_corrected = round(f_npq, 2)) %>% 
  select(date, station, year, pres, f_corrected)
...
ctd_integrated <- ctd_fl %>%
  ...
  arrange(pres) %>%
  summarise(
    integrated_fl = sum(f_corrected * c(diff(pres), 0), na.rm = TRUE),  # Simple integration
    .groups = 'drop'
  ) %>%
...
ggsave(here("figures", "panel_kc10_interp_int_f_no_npq.png"), combined_plot,
       width = 16, height = 8)
```
